...
- 说明：静态页默认用「📦 内置模拟数据」离线演示；把「数据来源」切到「🔌 连接真实后端」并按下节启动 `server.py`，即可走**真实抓取 + 真实人工闸门入库**。

### 4c. 真实后端 API（让静态页 / 任意前端调用真实管线）

`server.py` 是一个**零新增依赖**（仅标准库 `http.server`）的 JSON API，复用 `src/` 同一套核心逻辑，
把"真实数据获取 → 六维拆解 → 二创 → 人工确认入库"暴露成 HTTP 接口，供 `index.html`（或任意前端）调用。
...
启动后访问 `http://127.0.0.1:8000/` 会直接托管 `index.html`；在页面左侧把「数据来源」选为
**🔌 连接真实后端**（地址默认 `http://127.0.0.1:8000`）即可走真实管线。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/health` | 健康检查 |
| POST | `/api/analyze` | 入参 `{account, source}`，跑 获取→分析→生成，返回完整报告 + `session_id` |
| POST | `/api/revise` | 入参 `{session_id, feedback}`，**修改轮次**：按意见重生成候选 |
| POST | `/api/confirm` | 入参 `{session_id, selected[], operator}`，人工确认后入库，返回资产 ID |

合规设计：后端在 `analyze/revise` 时把草稿保存在服务器端会话里，`confirm` 时取出重新计算
**sha256 摘要**并调用 `HumanReviewGate.assert_approved()`——草稿被篡改或会话不匹配一律拒写，
与 CLI / Streamlit 版是同一套不可绕过的硬闸门。已开 CORS，可被 GitHub Pages 上的静态页跨域调用。
...
├── demo.py                      # 一键演示脚本
├── app.py                       # 可视化 Web Demo（Streamlit 单文件，复用 src 逻辑，含修改轮次）
├── index.html                   # 纯前端静态 Demo（双模式：内置模拟 / 连接真实后端）
├── server.py                    # 真实后端 API（标准库 http.server，零新增依赖，复用 src 管线）
